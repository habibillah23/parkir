    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 to-slate-950 shadow-[0_0_30px_-8px_rgba(148,163,184,0.4)] ring-1 ring-slate-700/50 p-6 max-w-lg">
        <form action="{{ route('transaksi.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="plat_nomor" value="{{ old('plat_nomor') }}" required autofocus
            ...
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">Nama Pemilik <span class="text-gray-500">(opsional)</span></label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">Area Parkir</label>
                <select name="area_parkir_id" required
                        class="w-full bg-slate-800/60 border border-slate-700 text-gray-100 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50">
                    <option value="">-- Pilih Area --</option>
                    @foreach ($areas as $area)
                        <option value="{{ $area->id }}" {{ old('area_parkir_id') == $area->id ? 'selected' : '' }}>
                            {{ $area->nama_area }} (sisa {{ $area->kapasitas - $area->kapasitasTerpakai() }} slot)
                        </option>
                    @endforeach
                </select>
            </div>
            <div class="flex gap-2 pt-2">
                ...
                <a href="{{ route('transaksi.index') }}" class="bg-slate-800 hover:bg-slate-700 text-gray-200 text-sm px-4 py-2 rounded-md border border-slate-700 transition-colors">
                    Batal
                </a>
            </div>
        </form>
